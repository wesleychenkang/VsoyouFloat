...
public class EmailLayoutView extends BaseLayout {
	...
	protected void onInitUI(Context ctx) {
		...
		edit_token.setTextSize(TypedValue.COMPLEX_UNIT_DIP,14);
    ...
    private boolean checkNumber (){
    	...
    	if(TextUtils.isEmpty(number)){
			Toast.makeText(getContext(), "请输入邮箱号码", Toast.LENGTH_SHORT).show();
			return false;
		 }
	...
	public void onClick(View v) {
		...
		if (v == btn_token) {
			if(checkNumber ()){
				...
				Toast.makeText(getContext(), "请输入正确格式的邮箱号码", Toast.LENGTH_SHORT).show();
			}
		}
		if(v==btn_confirm){
			checkToken();
